...
/// A streaming multipart/form-data parser that writes files directly to disk
pub const MultipartParser = struct {
    ...
    /// Get the next part in the multipart stream
    ...
    /// Stream the current part's content to a writer (e.g., file)
    pub fn streamPartContent(self: *MultipartParser, writer: std.io.AnyWriter, max_size: usize) !usize {
        ...
        while (true) {
            ...
            if (mem.indexOf(u8, accumulator.items, full_boundary)) |boundary_pos| {
                ...
                // Clear buffer and prepare for next part
                accumulator.clearRetainingCapacity();
                break;
            }

            // If accumulator is getting large, write some of it
...
/// Helper for handling multipart file uploads with streaming to disk
pub const FileUploadHandler = struct {
    upload_dir: []const u8,
    allocator: mem.Allocator,
    max_file_size: usize = 50 * 1024 * 1024, // 50MB default
    ...
    /// Process multipart upload and return list of uploaded files
