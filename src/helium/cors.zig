...
pub fn any(comptime ContextType: type) mw.chain.Types(ContextType).MiddlewareFn {
    return struct {
        fn handler(ctx: *ContextType, req: *Request, res: *Response, next_opaque: *anyopaque) !void {
            ...
            try res.headers.append(res.allocator, .{ .name = "Access-Control-Allow-Origin", .value = "*" });
            try res.headers.append(res.allocator, .{ .name = "Access-Control-Allow-Methods", .value = "GET, POST, PUT, DELETE, OPTIONS" });
            try res.headers.append(res.allocator, .{ .name = "Access-Control-Allow-Headers", .value = "Content-Type, Authorization" });
